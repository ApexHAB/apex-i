...
sub do_cut {
	...
	if ($@) {     # timed out
		...
		print $errorlog "Could not tell PIC to init cutdown, request timed out.\n";
	}
}
...
if ($@) {     # timed out
	die "Unexpected TIMEOUT ERROR" unless $@ eq "alarm\n";   # propagate unexpected errors
	print $errorlog "Could not retrieve time from PIC, request timed out.\n";
}
...
if ($@) {     # timed out
	die "Unexpected TIMEOUT ERROR" unless $@ eq "alarm\n";   # propagate unexpected errors
	print $errorlog "Could not retrieve APRS SUFFIX from PIC, request timed out.\n";
}
...
if (fileexists($logicfile) and $gps_alt<=$logic_down_lim) {
	...
	eval { #TIMEOUTSEQ
		local $SIG{ALRM} = sub { die "alarm\n" }; # NB: \n required
		alarm 10;
		$result = do_admon($admon, "C211\n");
		sleep(7);
		alarm 0;
	};
	if ($@) {     # timed out
		die "Unexpected TIMEOUT ERROR" unless $@ eq "alarm\n";   # propagate unexpected errors
		print $errorlog "Could not turn on light and siren, request timed out.\n";
	}
...
#if (fileexists($takepic)) {
	eval { #TIMEOUTSEQ
		local $SIG{ALRM} = sub { die "alarm\n" }; # NB: \n required
		alarm 6;
		do_admon($admon, "C3121\n");
		sleep(4);
		alarm 0;
	};
	if ($@) {     # timed out
		die "Unexpected TIMEOUT ERROR" unless $@ eq "alarm\n";   # propagate unexpected errors
		print $errorlog "Could not send TAKE PIC NOW command to PIC, request timed out.\n";
	}
	#system("rm $takepic"); # once done, delete the file
#}
